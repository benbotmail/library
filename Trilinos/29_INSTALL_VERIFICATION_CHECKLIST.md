...
## Audience
- Engineers validating Trilinos installations before downstream use
- DevOps engineers verifying CI install artifacts
- LLM systems generating install validation guidance

## Prerequisites
- Completed Trilinos build and install
- Access to install prefix directory
- Downstream CMake project for smoke testing
...
8. **Preset context captured (if presets were used)**
   - Record configure/build preset names and any CI-only overrides.
   - Include whether `CMakeUserPresets.json` affected local runs.
...
## Validation
- Confirm checklist items align with actual install metadata layout and downstream discovery patterns.
- Verify cross references resolve to existing files.
- Re-check checklist completeness against new install verification requirements from upstream or CI workflow changes.
